...
public extension NeuralNet {
    ...
    public convenience init(url: URL) throws {
        ...
        guard let layerNodeCounts = array[NeuralNet.layerNodeCountsKey] as? [Int],
            let lrDouble = array[NeuralNet.learningRateKey] as? Double,
            let momentumDouble = array[NeuralNet.momentumKey] as? Double,
            ...
            let weightsDouble = array[NeuralNet.weightsKey] as? [[Double]]
            else {
                throw Error.initialization("One or more required NeuralNet properties are missing.")
        }
        let lr = Float(lrDouble)
        let momentum = Float(momentumDouble)
        let weights = weightsDouble.map { $0.map { Float($0) } }
